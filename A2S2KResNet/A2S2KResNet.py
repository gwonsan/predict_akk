...
# wandb 초기화
wandb.init(project="A2S2K-ResNet", name="training_run")

# # Setting Params

parser = argparse.ArgumentParser(description='Training for HSI')
parser.add_argument(
    '-d', '--dataset', dest='dataset', default='GS', help="Name of dataset.")
parser.add_argument(
    '-o',
    '--optimizer',
    dest='optimizer',
...
parser.add_argument(
    '-e', '--epoch', type=int, dest='epoch', default=100, help="No of epoch")
parser.add_argument(
    '-i', '--iter', type=int, dest='iter', default=2, help="No of iter")
parser.add_argument(
...
def load_dataset(Dataset, split=0.9):
    ...
    if Dataset == 'IN':
        ...
        image_file = data_path + '202407_downsampled_gongsan.h5'
...
lr, num_epochs, batch_size = 0.0001, 200, 64
...
def train(net,
    ...
    for epoch in range(epochs):
        ...
        for X, y in train_iter:
            ...
            # 예측값과 실제값 저장
            train_preds.extend(y_hat.argmax(dim=1).cpu().numpy())
            train_labels.extend(y.cpu().numpy())
            
        lr_adjust.step()
        
        # 클래스별 precision과 recall 계산
        train_precision = metrics.precision_score(train_labels, train_preds, average=None)
        train_recall = metrics.recall_score(train_labels, train_preds, average=None)
        ...
        metrics_dict = {
            ...
            "val_loss": valida_loss,
            "val_acc": valida_acc
        }
        
        # 클래스별 precision과 recall 추가
        for i, (prec, rec) in enumerate(zip(train_precision, train_recall)):
            metrics_dict[f"class_{i}_precision"] = prec
            metrics_dict[f"class_{i}_recall"] = rec
        ...
        for i, (prec, rec) in enumerate(zip(train_precision, train_recall)):
            print(f"Class {i}: Precision = {prec:.3f}, Recall = {rec:.3f}")
...
for index_iter in range(ITER):
    ...
    torch.save(
        net.state_dict(), "./models/2407_new_S3KAIResNetpatch_" + str(img_rows) + '_' +
...
# 전체 데이터에 대한 예측 수행
print("전체 데이터에 대한 분류 수행 중...")
predictions = []
with torch.no_grad():
    for X, _ in all_iter:
        X = X.to(device)
        net.eval()
        pred = net(X).cpu().argmax(axis=1).numpy()
...
# classification_results 폴더가 없으면 생성
...
csv_path = './classification_results/' + 'S3KAIResNetpatch:' + str(img_rows) + '_' + Dataset + 'split' + str(VALIDATION_SPLIT) + '.csv'
...
print("분류 결과가 CSV 파일로 저장되었습니다:", csv_path)
print("분류 시각화 결과가 classification_maps 폴더에 저장되었습니다.")
