...
def load_dataset(Dataset, split=0.9):
    ...
    if Dataset == 'KSC':
        testing = sio.loadmat(data_path + 'gs_07.mat')
        gt_testing = sio.loadmat(data_path + 'gs_gt.mat')
        data_hsi = testing['data']
        gt_hsi = gt_testing['data']
        K = data_hsi.shape[2]
        TOTAL_SIZE = 227193
        VALIDATION_SPLIT = split
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
...
def train(net,
    ...
    for epoch in range(epochs):
        ...
        for X, y in train_iter:
            ...
            train_acc_sum += (y_hat.argmax() == y).sum().cpu().item()
            n += y.shape[0]
            batch_count += 1
        lr_adjust.step()
...
def sampling(proportion, ground_truth):
    ...
    for i in range(m):
        indexes = [
            j for j, x in enumerate(ground_truth.ravel().tolist()) #1차원화
            if x == i + 1
        ]
...
print('./report/' + 'S3KAIResNetpatch:' + str(img_rows) + '_' + Dataset + 'split'
...
record.record_output(
    ...
    './report/' + 'S3KAIResNetpatch_' + str(img_rows) + '_' + Dataset + 'split'
    + str(VALIDATION_SPLIT) + 'lr' + str(lr) + PARAM_OPTIM + '_kernel_' +
    str(PARAM_KERNEL_SIZE) + '.txt')

Utils.generate_png(
    all_iter, net, gt_hsi, Dataset, device, total_indices,
    './classification_maps/' + 'S3KAIResNetpatch_' + str(img_rows) + '_' +
